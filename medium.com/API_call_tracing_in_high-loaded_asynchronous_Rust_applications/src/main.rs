...
use serde_json::{to_value, Value as JSON_Value};
...
struct RpcResponse {
    ...
    result: Option<JSON_Value>,
    #[serde(skip_serializing_if = "Option::is_none")]
    error: Option<JSON_Value>,
}

async fn method_test(_payload: JSON_Value) -> Result<JSON_Value, String> {
    Ok(to_value("passed").unwrap())
}

async fn method_ls(payload: JSON_Value) -> Result<JSON_Value, String> {
    #[derive(Deserialize)]
...
async fn rpc_call<F, Fut>(f: F, payload: JSON_Value) -> RpcResponse
where
    F: Fn(JSON_Value) -> Fut,
    Fut: Future<Output = Result<JSON_Value, String>>,
{
    match f(payload).await {
        ...
        Err(e) => RpcResponse {
            result: None,
            error: Some(JSON_Value::String(e)),
        },
    }
}

fn call_scope<F>(trace: bool, f: F) -> (TaskLocalFuture<Option<mpsc::UnboundedSender<String>>, F>,
...
async fn handle(req: Request<Body>) -> Result<Response<Body>, Infallible> {
    ...
    if parts.method == Method::POST {
        if let Some(method) = parts.uri.path().strip_prefix("/api/") {
            ...
            let payload: JSON_Value = serde_json::from_slice(&hyper::body::to_bytes(&mut body).await.unwrap()).unwrap_or_default();
